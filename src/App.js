...
class App extends Component {
  ...
  onImageClick = thumbnailUrl => {
    ...
    const imageEl = imageValue[0].imageUrl
    console.log(imageEl)
  ...
  render() {
    const {score, time, imageUrl, category, isGameRunning} = this.state
      ...
      <div className="app-container">
        <nav className="nav-header">
          ...
          <li className="right-container">
            <p className="score-el">Score: {score}</p>
            <li className="time-display-containers">
              <img
                src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
                alt="timer"
                className="time-logo"
              />
              <p className="score-el">{time} sec</p>
            </li>
          </li>
        </nav>
        ...
        {isGameRunning ? (
          <div className="project-display-container">
            <ul>
              <img src={imageUrl} alt="match item" className="big-image" />
              <div className="tab-elements">
                {tabsList.map(eachTab => (
                  <li key={eachTab.tabId}>
                    <button
                      type="button"
                      onClick={() => this.onTabClick(eachTab.tabId)}
                      className={`button-el ${
                        eachTab.tabId === category ? 'style-btn' : ''
                      }`}
                    >
                      {eachTab.displayText}
                    </button>
                ...
                {thumbnailList.map(each => (
                  <li key={each.id}>
                    <button
                      ...
                      type="button"
                      onClick={() => this.onImageClick(each.thumbnailUrl)}
                    >
                      <img
          ...
          <div className="score-display-container">
            <div className="score-board">
              ...
              >
                <div className="play-btn-elements">
                  <img
